...
char	*ft_copy_stash(char *stash)
{
	...
	while (stash[i] && stash[i] != '\n')
		i++;
	stash_len = ft_strlen_gnl(stash) - i + 1;
	new_stash = malloc(sizeof(*new_stash) * stash_len);
	if (!new_stash)
		return (NULL);
	if (stash_len == 1)
		ft_strlcpy_gnl(new_stash, &(stash[i]), stash_len);
	else
		ft_strlcpy_gnl(new_stash, &(stash[i + 1]), stash_len);
	free(stash);
	return (new_stash);
}
